            ...
            <Page size={paperSize} style={styles.page}>
                    ...
                    {invoice.items && invoice.items.length > 0 ? (
                        invoice.items.map((item, i) => (
                            <View key={i} style={styles.tableRow}>
                                ...
                                <Text style={styles.colPrice}>{currencySymbol}{(item.unit_price || 0).toFixed(2)}</Text>
                                <Text style={styles.colTotal}>{currencySymbol}{((item.quantity || 0) * (item.unit_price || 0)).toFixed(2)}</Text>
                            </View>
                        ))
                    ) : (
                ...
                <View style={styles.summary}>
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>{currencySymbol}{(invoice.subtotal || 0).toLocaleString()}</Text>
                    </View>
                    {(invoice.tax_rate || 0) > 0 && (
                        <View style={styles.summaryRow}>
                            <Text style={styles.summaryLabel}>Tax ({invoice.tax_rate}%)</Text>
                            <Text style={styles.summaryValue}>{currencySymbol}{(invoice.tax_amount || 0).toLocaleString()}</Text>
                        </View>
                    )}
                    <View style={styles.summaryRow}>
                        <Text style={styles.summaryLabel}>Total</Text>
                        <Text style={[styles.summaryValue, { fontSize: 14, color: '#1E3A5F' }]}>{currencySymbol}{(invoice.total || 0).toLocaleString()}</Text>
                    </View>
                </View>
